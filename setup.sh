...
if [ ! -d "$CONFIGDIR" ]; then
    ...
    mkdir -p "$CONFIGDIR"
    echo -e "${GREEN}.config directory created: $CONFIGDIR${RC}"
fi
...
echo -e "${YELLOW}Cloning dxsbash repository into: $LINUXTOOLBOXDIR/dxsbash${RC}"
git clone https://github.com/digitalxs/dxsbash "$LINUXTOOLBOXDIR/dxsbash"
if [ $? -eq 0 ]; then
    echo -e "${GREEN}Successfully cloned dxsbash repository${RC}"
else
    echo -e "${RED}Failed to clone dxsbash repository${RC}"
    exit 1
fi
...
installDepend() {
    ...
    if [ "$PACKAGER" = "pacman" ]; then
        if ! command_exists yay && ! command_exists paru; then
            echo -e "Installing yay as AUR helper..."
            ${SUDO_CMD} ${PACKAGER} --noconfirm -S base-devel
            cd /opt && ${SUDO_CMD} git clone https://aur.archlinux.org/yay-git.git && ${SUDO_CMD} chown -R "${USER}:${USER}" ./yay-git
            cd yay-git && makepkg --noconfirm -si
        else
            echo -e "AUR helper already installed"
        fi
        if command_exists yay; then
            ...
            echo -e "No AUR helper found. Please install yay or paru."
            exit 1
        fi
    ...
    if fc-list :family | grep -iq "$FONT_NAME"; then
        echo -e "Font '$FONT_NAME' is installed."
    else
        echo -e "Installing font '$FONT_NAME'"
        ...
        if wget -q --spider "$FONT_URL"; then
            ...
            rm -rf "${TEMP_DIR}"
            echo -e "'$FONT_NAME' installed successfully."
        else
            echo -e "Font '$FONT_NAME' not installed. Font URL is not accessible."
        fi
    fi
}
...
installZoxide() {
    if command_exists zoxide; then
        echo -e "Zoxide already installed"
        return
    fi
...
EOL
        ...
        if [ ! -f "/usr/share/zsh-autosuggestions/zsh-autosuggestions.zsh" ] && [ ! -f "$ZSH_PLUGIN_DIR/zsh-autosuggestions/zsh-autosuggestions.zsh" ]; then
            ...
            # Add to .zsh_plugins
            echo -e "# Manual installation" >> "$USER_HOME/.zsh_plugins"
            echo -e "source $PLUGIN_DIR/zsh-autosuggestions/zsh-autosuggestions.zsh" >> "$USER_HOME/.zsh_plugins"
        fi
        ...
        if [ ! -f "/usr/share/zsh-syntax-highlighting/zsh-syntax-highlighting.zsh" ] && [ ! -f "$ZSH_PLUGIN_DIR/zsh-syntax-highlighting/zsh-syntax-highlighting.zsh" ]; then
            ...
            # Add to .zsh_plugins
            echo -e "source $PLUGIN_DIR/zsh-syntax-highlighting/zsh-syntax-highlighting.zsh" >> "$USER_HOME/.zsh_plugins"
        fi
...
setupShellConfig() {
    ...
    echo -e "${YELLOW}Setting up configuration for $SELECTED_SHELL...${RC}"
    ...
    if [ "$SELECTED_SHELL" = "bash" ]; then
        if [ -e "$USER_HOME/.bashrc" ]; then
            ...
            echo -e "${YELLOW}Moving old bash config file to $BACKUP_FILE${RC}"
    ...
    if [ -f "$GITPATH/.bash_aliases" ]; then
        ...
        echo -e "# Custom user aliases - Created by dxsbash setup" > "$USER_HOME/.bash_aliases"
        echo -e "# Add your personal aliases below" >> "$USER_HOME/.bash_aliases"
        echo -e "" >> "$USER_HOME/.bash_aliases"
        ...
        if [ -e "$USER_HOME/.zshrc" ]; then
            ...
            echo -e "${YELLOW}Moving old zsh config file to $BACKUP_FILE${RC}"
        ...
        echo -e "${GREEN}Installing dxsbash .zshrc file${RC}"
        ...
        if [ -f "$USER_HOME/.zsh_plugins" ]; then
            ...
            if ! grep -q "source ~/.zsh_plugins" "$USER_HOME/.zshrc"; then
                ...
                echo -e "" >> "$USER_HOME/.zshrc"
                echo -e "# Source plugins" >> "$USER_HOME/.zshrc"
                echo -e "[ -f ~/.zsh_plugins ] && source ~/.zsh_plugins" >> "$USER_HOME/.zshrc"
            fi
        fi
        ...
        if [ -e "$USER_HOME/.config/fish/config.fish" ]; then
            ...
            echo -e "${YELLOW}Moving old fish config file to $BACKUP_FILE${RC}"
...
setDefaultShell() {
    echo -e "${YELLOW}Setting $SELECTED_SHELL as your default shell...${RC}"
    ...
    if [ -z "$SHELL_PATH" ]; then
        echo -e "${RED}Error: Could not find path to $SELECTED_SHELL. Please set it as your default shell manually.${RC}"
        return 1
    fi
    ...
    if [ $? -eq 0 ]; then
        ...
        echo -e "${RED}Failed to set $SELECTED_SHELL as your default shell. Please do it manually with:${RC}"
        echo -e "${YELLOW}chsh -s $SHELL_PATH${RC}"
    fi
}

installResetScript() {
    echo -e "${YELLOW}Installing reset-shell-profile script...${RC}"
    ...
    if [ -f "$GITPATH/reset-bash-profile.sh" ]; then
        ...
        echo -e "${GREEN}Reset script installed successfully at $LINUXTOOLBOXDIR/reset-*-profile.sh${RC}"
        echo -e "${GREEN}You can run it with: sudo reset-shell-profile [username]${RC}"
    else
...
installUpdaterCommand() {
    echo -e "${YELLOW}Installing dxsbash updater script...${RC}"
    ...
    if [ -f "$GITPATH/updater.sh" ]; then
        ...
        echo -e "${GREEN}Updater script installed successfully at $LINUXTOOLBOXDIR/updater.sh${RC}"
        echo -e "${GREEN}You can update dxsbash anytime by running: upbashdxs${RC}"
    else
...
detectDistro() {
    ...
    if [ -f /etc/fedora-release ]; then
        DISTRO="fedora"
        echo -e "${BLUE}Detected Fedora Linux${RC}"
    elif [ -f /etc/redhat-release ]; then
        if grep -q "CentOS" /etc/redhat-release; then
            DISTRO="centos"
            echo -e "${BLUE}Detected CentOS Linux${RC}"
        elif grep -q "Red Hat Enterprise Linux" /etc/redhat-release; then
            DISTRO="rhel"
            echo -e "${BLUE}Detected Red Hat Enterprise Linux${RC}"
        else
            DISTRO="redhat-based"
            echo -e "${BLUE}Detected Red Hat-based Linux${RC}"
        fi
    elif [ -f /etc/lsb-release ] && grep -q "Ubuntu" /etc/lsb-release; then
        DISTRO="ubuntu"
        echo -e "${BLUE}Detected Ubuntu Linux${RC}"
    elif [ -f /etc/debian_version ]; then
        DISTRO="debian"
        echo -e "${BLUE}Detected Debian Linux${RC}"
    elif [ -f /etc/arch-release ]; then
        DISTRO="arch"
        echo -e "${BLUE}Detected Arch Linux${RC}"
    elif [ -f /etc/SuSE-release ] || [ -f /etc/opensuse-release ]; then
        DISTRO="suse"
        echo -e "${BLUE}Detected SUSE Linux${RC}"
    else
        # Generic detection
        if command_exists apt; then
            DISTRO="debian-based"
            echo -e "${BLUE}Detected Debian-based Linux${RC}"
        elif command_exists dnf; then
            DISTRO="fedora-based"
            echo -e "${BLUE}Detected Fedora-based Linux${RC}"
        elif command_exists pacman; then
            DISTRO="arch-based"
            echo -e "${BLUE}Detected Arch-based Linux${RC}"
        elif command_exists zypper; then
            DISTRO="suse-based"
            echo -e "${BLUE}Detected SUSE-based Linux${RC}"
        fi
    fi
}
...
selectShell() {
    ...
    echo -e "${CYAN}==================================================${RC}"
    echo -e "${YELLOW}1) Bash ${RC}(default, most compatible)"
    echo -e "${YELLOW}2) Zsh ${RC}(enhanced features, popular alternative)"
    echo -e "${YELLOW}3) Fish ${RC}(modern, user-friendly, less POSIX-compatible)"
    echo ""
    ...
    echo -e "${GREEN}Selected shell: $SELECTED_SHELL${RC}"
}
...
configure_kde_terminal_emulators() {
    ...
    if [ "$XDG_CURRENT_DESKTOP" = "KDE" ] || command_exists konsole; then
        echo -e "${YELLOW}KDE environment detected, configuring terminal emulators...${RC}"
...
echo -e "${YELLOW}Made by Luis Miguel P.Freitas - DigitalXS.ca - 2025${RC}"
echo -e "${CYAN}$SELECTED_SHELL${RC}"
