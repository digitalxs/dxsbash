...
show_summary() {
    ...
    echo -e "  ${BRIGHT_YELLOW}2.${RESET} ${WHITE}Or run this command to use it right now:${RESET} ${BRIGHT_GREEN}$SELECTED_SHELL${RESET}"
    echo
...
# Run main function
main "$@"            cd yay-git && makepkg --noconfirm -si > /dev/null 2>&1
    ...
    for pkg in bash git curl wget neovim; do
    ...
    if fc-list :family | grep -iq "$FONT_NAME"; then
        ...
        if wget -q --spider "$FONT_URL"; then
            ...
            mv "${TEMP_DIR}"/*.ttf "$FONT_DIR"/"$FONT_NAME" > /dev/null 2>&1
    ...
    if [ "$SELECTED_SHELL" = "bash" ]; then
        ...
        if [ -f "$GITPATH/.bash_aliases" ]; then
            ...
            if [ -f "$USER_HOME/.zsh_aliases" ]; then
                ...
                stop_spinner "warning" "Failed to set up zsh aliases"    elif [ -f /etc/lsb-release ] && grep -q "Ubuntu" /etc/lsb-release; then
        DISTRO="ubuntu"
...
checkEnv() {
    ...
    if [ $found_group -eq 0 ]; then
        error_message "You need to be a member of a superuser group"
        exit 1
    fi
